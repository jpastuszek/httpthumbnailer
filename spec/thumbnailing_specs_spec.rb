...
describe ThumbnailingSpecs do
	it "can be created from URI" do
		ts = ThumbnailingSpecs.from_string('test,128,256,jpeg,magick:64,number:8/pad,128,128,png,background:0xFF00FF/crop,32,32,gif')
	...
	it 'should provide :input symbol when input is used as width, height or format' do
		ts = ThumbnailingSpecs.from_string('test,input,256,jpeg,magick:64,number:8/pad,128,input,png,background:0xFF00FF/crop,32,32,input')
	...
	it 'should provide editing specs' do
		ts = ThumbnailingSpecs.from_string('test,input,256,jpeg,magick:64,number:8!blur,1,2,abc:x,xyz:2!cut,1,2,3,4/pad,128,input,png,background:0xFF00FF!blur,2,2,abc:x,xyz:2!cut,2,2,3,4/crop,32,32,input!blur,3,2,abc:x,xyz:2!cut,3,2,3,4')

		ts[0].edits[0].name.should == 'blur'
		ts[0].edits[0].args.should == ['1', '2']
		ts[0].edits[0].options.should == {'abc' => 'x', 'xyz' => '2'}
		ts[0].edits[1].name.should == 'cut'
		ts[0].edits[1].args.should == ['1', '2', '3', '4']

		ts[1].edits[0].name.should == 'blur'
		ts[1].edits[0].args.should == ['2', '2']
		ts[1].edits[0].options.should == {'abc' => 'x', 'xyz' => '2'}
		ts[1].edits[1].name.should == 'cut'
		ts[1].edits[1].args.should == ['2', '2', '3', '4']

		ts[2].edits[0].name.should == 'blur'
		ts[2].edits[0].args.should == ['3', '2']
		ts[2].edits[0].options.should == {'abc' => 'x', 'xyz' => '2'}
